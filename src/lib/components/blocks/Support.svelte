...
>
	...
	<div class="gap flex flex-1 justify-between gap-10">
		<div class="relative flex-1 pt-40">
			<Sun
				newClass="absolute w-20 h-20 left-[15%] -top-10 animate-once animate-delay-1000 {isInView
					? 'animate-ping'
					: 'opacity-0'}"
			/>
			{#if isInView}
				<img
					...
					class="absolute right-2 top-20 w-8/12 animate-delay-[800ms] {isInView
						? 'animate-fade'
						: ''}"
				/>
			{/if}
			<div class="h-[26rem]">
				{#if isInView}
					<img
						...
						class="left h-full w-full rounded-2xl object-cover animate-delay-[600ms] {isInView
							? 'animate-fade'
							: ''}"
					/>
				{/if}
		...
		<div class="relative flex-1">
			<div class="relative h-[26rem]">
				{#if isInView}
					<img
						...
						class="h-full w-full rounded-2xl object-cover animate-delay-[1500ms] {isInView
							? 'animate-fade'
							: ''}"
					/>
					<img
						...
						class="absolute left-8 top-full w-48 -translate-y-1/2 transform animate-delay-[1700ms] {isInView
							? 'animate-fade'
							: ''}"
					/>
				{/if}
			...
			<Sun
				newClass="absolute w-36 h-36 left-0 top-full -mt-8 animate-once animate-delay-[1900ms] {isInView
					? 'animate-ping'
					: 'opacity-0'}"
				color="#00FFDA"
			/>
