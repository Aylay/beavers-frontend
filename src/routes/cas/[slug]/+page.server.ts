...
export const load = (async ({ fetch, params }) => {
	...
	if (useCaseData.data) {
		...
		const articlesResponse = await fetch(import.meta.env.VITE_STRAPI_URL + '/api/articles?filters[category][slug][$eq]=' + content.attributes.categories.data[0].attributes.slug + '&sort=publishedAt%3Adesc&filters[publishedAt][$notNull]=true&pagination[pageSize]=3&fields[0]=slug&fields[1]=date&fields[2]=publishedAt&fields[3]=title&fields[4]=excerpt&populate[category][fields][0]=slug&populate[category][fields][1]=title&populate[mainImg][fields][0]=formats&populate[mainImg][fields][1]=url&populate[mainImg][fields][2]=alternativeText', {
			method: 'GET'
		})
