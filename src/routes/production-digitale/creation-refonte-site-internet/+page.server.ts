...
export const load: PageServerLoad = async ({ fetch }) => {
  ...
  useCases = useCases.filter((elm: any) => {
    if (elm.attributes.categories.data[0].attributes.title === 'Site internet') {
      return elm
    }
    if (elm.attributes.categories.data[1] && elm.attributes.categories.data[1].attributes.title === 'Site internet') {
      return elm
    }
  })
